...
首页可切换原版，以及「证据与记忆」「比较与反向检查」「证据驱动的行动比较」三个实验版本。四个人共用决策流程，保留各自性格：核对公开证据 → 竞争性解释 → 候选行动 → 风险/收益比较 → 公开表达 → 根据实际行动和新记录更新私人判断。规划摘要不进入游戏事件，规划调用计入“思考”账单；规划总预算 20 秒，失败时回到原版表达。

`/lab` 在 `ENABLE_GOD_VIEW=true` 时开放，使用首页保存的模型设置运行固定对照。预设限制单次 24 次公开行动，另有规划、重试和评分调用。任务 ID 存在 URL，可刷新恢复；服务重启后内存报告清空，可打开完整 JSON 保存。报告含输出、失败、用量、耗时和配置指纹，不含 API Key。

本次已完成 3 轮比较和留出复测，未证明实验版本优于原版，因此默认仍为原版。详见 [设计与实际结果](docs/experiments/2026-09-19-thinking-results.md)。可运行 `node scripts/summarize-experiment.mjs docs/experiments/2026-09-19-holdout.json` 复算汇总。评分是同模型的主观盲评，不代表整局胜率。同一轮投票对所有版本均使用冻结快照，避免后投的人照抄本轮前面的票。
...
2. 发言轮：存活玩家按座位号顺序用 1~2 句生活口语描述自己的词，不得念出词面（服务端会程序化校验，失败后带纠正反馈重试，最多 5 次尝试）。
...
- 发言会持续尝试直到校验通过，最多请求 5 次（含首次），每次发言总等待上限 90 秒。网络错误、超时、429 / 5xx 按 500ms、1000ms、2000ms、4000ms 退避；发言层统一管理尝试次数，不再与客户端重试叠加。
...
- 投票保留原有策略：模型调用层对网络错误 / 超时 / 429 / 5xx 最多重试 2 次；Agent 输出不合格时再尝试 1 次，仍失败则随机选择合法候选并标红。
