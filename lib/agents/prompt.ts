...
function speechStrategy(view: AgentView): string {
  ...
  return [
    ...
    // 自觉少数时的伪装只靠这段提示词，服务端不注入任何身份信息。
    ...(hasPriorSpeech
      ? ['再掂量一句自己可能是多数还是少数。觉得自己可能是少数那方时，公开发言优先借用多数人已经用过的说法框架，只把它换成你自己的角度：选与自己词也相符的共通经历接话，不要主动强调“你们说的我都对不上”，也不要硬编自己的词没有的特点，更不能再抛只有你的词才成立的独特点。']
      : []),
...
function thoughtPlan(view: AgentView): string {
  ...
  return 'thought 写 2~4 句你真正的判断，按这个顺序写：先点名谁的话听起来和你的词像是同一边，再点名谁明显和你对不上，然后用一句话判断自己这轮可能是多数还是少数——若判断自己可能是少数，就写清伪装策略：贴谁的说法、避开哪个独特点，最后才说你这轮打算藏什么、怎么说。这里可以直接写出你的词，因为不会给任何人看。';
}
